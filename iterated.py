...
class IteratedKalmanCaterpillar(QCAlgorithm):
    def Initialize(self):
    ...
    def TradeBasis(self):
        ...
        self.Plot("Iterated EKF Analysis", "Basis Prediction", basis_prediction)
        self.Plot("Iterated EKF Analysis", "Actual Basis", current_basis)
        self.Plot("Iterated EKF Analysis", "Velocity", state_pred[1])
        self.Plot("Z-Score", "Value", z)

        # --- 3. ITERATED EKF UPDATE ---
        state_iter = state_pred.copy()
        iteration_count = 0

        for i in range(self.max_iterations):
            H = self.ObservationJacobian(state_iter)
            innovation = current_basis - self.ObservationModel(state_iter) + H @ (state_iter - state_pred)
            ...
            if np.linalg.norm(next_state - state_iter) < self.iteration_tolerance:
        ...
        self.Plot("Iterated EKF Analysis", "Iterations", iteration_count)
